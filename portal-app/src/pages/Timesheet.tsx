...
const Timesheet: React.FC = () => {
  ...
  const fetchTimesheet = async () => {
    ...
    try {
      ...
      // Build base days from existing timesheet or empty object
      const existingDays: Record<number, { type: TimesheetDayType; description: string }> =
        data?.days || {};
      const autoDays = { ...existingDays };

      // Calculate month date range for querying
      const monthStart = new Date(year, month, 1);
      const monthEnd = new Date(year, month + 1, 0);
      const pad = (n: number) => String(n).padStart(2, '0');
      const monthStartStr = `${year}-${pad(monthStart.getMonth() + 1)}-01`;
      const monthEndStr = `${year}-${pad(monthStart.getMonth() + 1)}-${pad(monthEnd.getDate())}`;

      // Fetch approved leave requests overlapping with this month
          ...
          while (cur <= to) {
            if (cur.getFullYear() === year && cur.getMonth() === month) {
              ...
              if (!autoDays[day]) {
                autoDays[day] = {
                  type: 'Leave',
                  description: leave.reason || leave.leave_type || 'Approved Leave',
                };
              }
            }
      ...
      // Fetch national holidays in this month
      ...
      if (data) {
        setTimesheet({ ...data, days: autoDays });
      } else {
        setTimesheet({
          id: '',
