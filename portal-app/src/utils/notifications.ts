...
function buildPushContent(payload: NotificationPayload): { title: string; body: string; url: string } {
  ...
  switch (payload.type) {
    ...
    case 'leave_applied_by_manager':
      return {
        title: 'Leave applied on your behalf',
        body: `Your manager applied ${d.leave_type} leave for you (${d.from_date} – ${d.to_date})`,
        url: '/leaves',
      };
    case 'leave_awarded':
      return {
        title: 'Extra leave awarded',
        body: `Your manager added ${d.days} day(s) to your Paid & Sick leave balance`,
        url: '/leaves',
      };
